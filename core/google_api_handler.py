...
def load_doc_content(doc_id, as_html=True):
    ...
    try:
        ...
        for element in body_content:
            if 'paragraph' in element:
                for pe in element.get('paragraph').get('elements'):
                    if 'textRun' in pe:
                        plain_text += pe.get('textRun').get('content')

        if not as_html:
            return title, plain_text.strip()

        html_content = markdown.markdown(plain_text, extensions=['fenced_code', 'codehilite', 'tables', 'nl2br'])
        ...
        final_html = f'<html><head><meta charset="UTF-8">{CSS_STYLE}</head><body>{html_content}</body></html>'
